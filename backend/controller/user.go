...
func CreateUser(c *gin.Context) {
	...
	// fmt.Printf(("user.Civ = %v, len = "), user.Civ)
	// fmt.Println(len(user.Civ))
	//check civ
	if len(user.Civ) != 13 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Civ not complete"})
		return
	}

	// fmt.Printf(("user.Phone = %v, len = "), user.Phone)
	// fmt.Println(len(user.Phone))
	//check phone
	if len(user.Phone) != 10 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Phone not complete"})
		return
	}
